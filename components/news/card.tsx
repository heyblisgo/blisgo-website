...
export const Card = ({ id, attributes }: News) => {
  const { title, contents, media } = attributes;
  return (
    <article className="flex md:flex-col flex-row gap-4 max-md:py-4">
      <Link href={`/news/${id}`} className="bg-gray-200 max-md:w-[120px] max-md:flex-shrink-0 aspect-square h-fit md:w-[296px]">
        <img src={media.data[0].attributes.formats.small.url} alt={title} className="object-cover w-full h-full" />
      </Link>
      <div className="flex flex-col gap-2 max-md:justify-between md:hidden">
